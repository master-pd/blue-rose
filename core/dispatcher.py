...
class Dispatcher:
    ...
    def register_handler(self, handler_type: str, handler: Callable, filters: Optional[List] = None):
    ...
    async def dispatch(self, update: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # If no handlers matched, try fallback
    ...
    async def _handle_no_match(self, update: Dict[str, Any], update_type: str) -> Optional[Dict[str, Any]]:
        ...
        # Default handling for certain update types
        if update_type == 'text_message':
            # Could trigger AI response or default reply
            pass
        ...
        if handler_type in self.handlers:
            for handler_info in self.handlers[handler_type]:
                if handler_info['handler'] == handler:
                    ...
                    status = "enabled" if enable else "disabled"
                    logger.info(f"{status.capitalized()} handler: {handler.__name__}")
                    return True
        return False
